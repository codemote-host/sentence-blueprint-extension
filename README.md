...
> 0.6.0：使用 Apache Doris 官方技术文档建立长句回归集，补齐名词片段、并列主句、`so` 结果分句、省略状语从句及并列 doing 结构；同时重做长文卡片的字体层级与深浅主题可读性。
...
- “主干”保留完整语法成分；“主干语义”进一步去掉定语、状语、同位语及补充性并列，便于先抓住核心命题。
- 并列连词会在结构视图中单独标注；`and so does ...` 会识别为“并列省略分句”，并提示 `does` 代替前一分句的谓语内容。
- `, so + 完整主谓` 会按结果并列分句处理；无逗号的目的/结果状语从句与 `so ... that ...` 不会被误切成并列主句。
- `When deploying ...` 等省略结构会标为“状语从句（省略）”，并为非谓语核心继承主句逻辑主语。
- `either ... or ...` 会保留两侧完整并列选项；逗号后的 `encoding, compressing, and reading ...` 会从宾语中分离为共享主语的并列 doing 结构。
- `Frontend (FE): receives ..., parses ...` 这类冒号项目符号会按“共享主语 + 并列谓语”展示，并保留每个谓语自己的宾语。
- 没有限定谓语的标题、列表项和名词短语会明确标为“句子片段”，不再强行生成 SV 主干。
...
- 并列词性会结合 `conj/cc` 依存关系做保守校正，如 `on or off` 统一为副词；Stanza 的原始词性仍保留用于排错。
- 支持右键或 `Alt+Shift+S` 拆解当前选中的英文。
- 本地服务优先使用 Stanford Stanza，并保留基础规则作为服务异常时的降级方案。
- 多句文本会自动分句并逐句展示，原始依存关系和成分树随分析结果缓存。
- 复杂句的结构视图仍会保留 `Personally`、`in mind` 等句内状语标注，不会被默认“主句”色块覆盖。
- 使用 SQLite 缓存已分析句子。

本轮解析回归样本取自 [Apache Doris 官方 Overview 与 Typical Use Cases](https://doris.apache.org/docs/4.x/getting-started/what-is-apache-doris#typical-use-cases)，覆盖技术文档中常见的长定语、并列结构、被动语态、标题片段和补充性非谓语。
